...
def start_spark(app_name='my_spark_app', master='local[*]', jar_packages=[],
    ...
    if not flag_debug:
        ...
        spark_builder = (
            SparkSession
                .builder.config("spark.driver.extraClassPath","C:\Program Files (x86)\MySQL\Connector J "
                                                                     "8.0\mysql-connector-java-8.0.24.jar")
                .appName(app_name))
    else:
        # get Spark session factory
        spark_builder = (
            SparkSession
                .builder
                .master(master).config("spark.driver.extraClassPath","C:\Program Files (x86)\MySQL\Connector J "
                                                                     "8.0\mysql-connector-java-8.0.24.jar")
                .appName(app_name))
